...
public class A2AServerConfiguration {
    ...
    private AgentCard createTranslationAgentCard() {
        AgentProvider provider = new AgentProvider("Google", "https://google.com");
        ...
        AgentSkill skill = new AgentSkill(
                "ai-translator", "AI Translation Service",
                "Professional AI translator supporting multiple languages. Can translate text between various " +
                        "language pairs including English, Chinese, Japanese, French, Spanish, German, and more.",
                List.of("translation", "language", "ai", "multilingual"), List.of(
                "Example: Translate 'Hello World' to Chinese", "Example: 请把这句话翻译成英文: '你好'",
        ...
        return new AgentCard(
                "AI Translation Bot",
                "Professional AI translation service powered by advanced language models. Supports translation " +
                        "between multiple languages with high accuracy and context awareness.",
                "http://localhost:8888/a2a", provider, "1.0.0", "http://localhost:8888/docs", capabilities,
                authentication, List.of("text"), List.of("text"), List.of(skill)
        );
    }
    ...
    private TaskHandler createTranslationTaskHandler(ChatModel chatModel) {
        ...
        return (task, message) -> {
            try {
                ...
                String translatedText = chatClient.prompt(translationPrompt)
                                                  .call()
                                                  .content();
                ...
                Message responseMessage = new Message(
                        UUID.randomUUID()
                            .toString(), "message", "assistant", List.of(responsePart), message.contextId(), task.id(),
                        List.of(message.messageId()), null
                );

                // Create completed status
                TaskStatus completedStatus = new TaskStatus(
                        TaskState.COMPLETED, null,  // No status message
                        Instant.now()
                               .toString()
                );
                ...
                List<Message> updatedHistory = task.history() != null
                        ...
                        : List.of();
                updatedHistory = List.of(java.util.stream.Stream.concat(
                ...
                return new Task(
                        task.id(), task.contextId(), task.kind(), completedStatus, task.artifacts(), updatedHistory,
                        task.metadata()
                );
    ...
    private String createTranslationPrompt(String text) {
        return String.format(
                """
                        You are a professional translator. Detect the source language and translate to the most appropriate target language.
                        
                        Instructions:
                        1. If the text is in Chinese, translate to English
                        2. If the text is in English, translate to Chinese
                        3. If the text is in French, translate to Chinese
                        4. If the text is in Spanish, translate to English
                        5. If the text is in German, translate to English
                        6. For other languages, automatically detect and translate to English or Chinese (whichever is more appropriate)
                        7. Maintain the original meaning and context
                        8. Provide natural, fluent translations
                        9. Only return the translated text, no explanations
    ...
    private Task createErrorTask(Task originalTask, String errorMessage) {
        TaskStatus errorStatus = new TaskStatus(
                TaskState.FAILED, null,  // No status message
                Instant.now()
                       .toString()
        );

        return new Task(
                originalTask.id(), originalTask.contextId(), originalTask.kind(), errorStatus, originalTask.artifacts(),
                originalTask.history(), originalTask.metadata()
        );
    }
}
