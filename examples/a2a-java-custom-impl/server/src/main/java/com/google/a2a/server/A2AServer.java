...
public class A2AServer {
    ...
    public JSONRPCResponse handleTaskSend(JSONRPCRequest request) {
        try {
            ...
            Task task = new Task(
                    params.id(), contextId, "task",  // kind is always "task"
                    initialStatus, null,    // No artifacts initially
        ...
        try {
            ...
            if (params.historyLength() != null && params.historyLength() > 0) {
                ...
                Task taskWithHistory =
                        new Task(
                                task.id(), task.contextId(), task.kind(), task.status(), task.artifacts(),
                                limitedHistory, task.metadata()
                        );
    ...
    public JSONRPCResponse handleTaskCancel(JSONRPCRequest request) {
        try {
            ...
            TaskStatus canceledStatus = new TaskStatus(
                    TaskState.CANCELED, null,  // No message
                    Instant.now()
                           .toString()
            );
            ...
            Task canceledTask =
                    new Task(
                            task.id(), task.contextId(), task.kind(), canceledStatus, task.artifacts(), task.history(),
                            task.metadata()
                    );
