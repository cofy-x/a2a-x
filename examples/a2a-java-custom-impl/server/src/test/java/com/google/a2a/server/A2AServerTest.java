...
class A2AServerTest {
    ...
    void setUp() {
        ...
        AgentCard agentCard = new AgentCard(
                "Test Agent", "Test Agent", "http://localhost:8888/a2a", null, "1.0.0", null,
                new AgentCapabilities(true, true, true), null, List.of("text"), List.of("text"), List.of()
        );
        ...
        TaskHandler taskHandler = (task, message) -> {
            ...
            TaskStatus status = new TaskStatus(
                    targetState, null,  // No additional message
                    Instant.now()
                           .toString()
            );

            return new Task(
                    task.id(), task.contextId(), task.kind(), status, task.artifacts(), task.history(),
                    task.metadata()
            );
        };
    ...
    void testHandleTaskSend() {
        ...
        Map<String, Object> params = Map.of(
                "id", "test-task-1", "message", Map.of(
                        "messageId", testMessage.messageId(), "kind", testMessage.kind(), "role", testMessage.role(),
                        "parts", List.of(Map.of("kind", "text", "text", "Hello, world!"))
                ), "metadata", Map.of()
        );

        JSONRPCRequest request = new JSONRPCRequest("request-1", "2.0", "message/send", params);
        ...
        assertEquals(
                TaskState.COMPLETED, resultTask.status()
                                               .state()
        );
        assertNotNull(resultTask.contextId());
        assertEquals("task", resultTask.kind());
        assertNotNull(resultTask.status()
                                .timestamp());
    }
    ...
    void testHandleTaskGet() {
        ...
        Message testMessage = new Message(
                UUID.randomUUID()
                    .toString(), "message", "user", List.of(textPart), null, null, null, null
        );

        // Send a task first
        Map<String, Object> sendParams = Map.of(
                "id", "test-task-2", "message", Map.of(
                        "messageId", testMessage.messageId(), "kind", testMessage.kind(), "role", testMessage.role(),
                        "parts", List.of(Map.of("kind", "text", "text", "Test message"))
                ), "metadata", Map.of()
        );

        JSONRPCRequest sendRequest = new JSONRPCRequest("request-1", "2.0", "message/send", sendParams);

        server.handleTaskSend(sendRequest);

        // Then query the task
        Map<String, Object> getParams = Map.of("id", "test-task-2");
        JSONRPCRequest getRequest = new JSONRPCRequest("request-2", "2.0", "tasks/get", getParams);
    ...
    void testHandleTaskCancel() {
        ...
        JSONRPCRequest cancelRequest = new JSONRPCRequest("request-2", "2.0", "tasks/cancel", cancelParams);
        ...
        assertEquals(
                TaskState.CANCELED, resultTask.status()
                                              .state()
        );
        assertNotNull(resultTask.status()
                                .timestamp());
    ...
    void testHandleTaskGetNotFound() {
        ...
        JSONRPCRequest request = new JSONRPCRequest("request-1", "2.0", "tasks/get", params);
        ...
        assertEquals(
                ErrorCode.TASK_NOT_FOUND.getValue(), response.error()
                                                             .code()
        );
        assertEquals(
                "Task not found", response.error()
                                          .message()
        );
    }
